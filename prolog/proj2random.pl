...
constructOutputString(State, TpeIdx, Tape, Otpt) :-
    ...
    string_chars(State, StateArr),
    append(OtptList, StateArr, OtptList1),
...
turingMachineforAllRandomized(_,_,_,[],[]).
turingMachineforAllRandomized(State, TapeIndex, Tape, Steps, Order) :-
    /** Randomly pick Next step */
    length(Steps, OptCnt),
    UpprBnd is OptCnt - 1,
    random_between(0,UpprBnd,NextIdx),
    nth0(NextIdx, Steps, NextStep),
    /** Remove Next Step from Steps pool */
    take(NextIdx, Steps, NStepsP1),
    NewIdx is NextIdx + 1,
    drop(NewIdx, Steps, NStepsP2),
    append(NStepsP1, NStepsP2, NSteps),
    ...
    (TmpLen == 0 -> 
        ...
        call(Op, State, TapeIndex, Tape, NSteps, Order)
    ; 
